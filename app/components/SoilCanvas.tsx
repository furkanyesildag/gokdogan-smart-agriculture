 ...
 * canlı HUD (pH / nem / N / taranan %). Orijinal dc-runtime sahnesinin birebir portu.
 */
export default function SoilCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
        ...
        for (let i = 0; i < COLS; i++) {
          ...
          if (rv > 0.01) {
            ...
            ctx.fillStyle = `rgba(${r},${g},${bl},${
              0.16 + rv * 0.5 * (0.4 + v * 0.6)
            })`;
            ctx.fill();
          }
          ctx.strokeStyle = "rgba(120,200,214,0.07)";
          ctx.lineWidth = 1;
          ctx.stroke();
        }
      }
      ...
      grd.addColorStop(0, "rgba(60,210,224,0.32)");
      grd.addColorStop(1, "rgba(60,210,224,0)");
      ctx.fillStyle = grd;
      ctx.beginPath();
      ctx.arc(rp.x, rp.y, tw * 2.4, 0, Math.PI * 2);
      ctx.fill();
      const rr = tw * (0.6 + pulse * 1.8);
      ctx.strokeStyle = `rgba(90,220,232,${0.5 * (1 - pulse)})`;
      ctx.lineWidth = 1.4;
      ctx.beginPath();
      ...
      bg2.addColorStop(0, "#f2f7f9");
      bg2.addColorStop(1, "#aebbc2");
      ctx.fillStyle = bg2;
      ...
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = "rgba(20,40,48,0.85)";
      ...
      glow.addColorStop(0, "rgba(120,230,240,1)");
      glow.addColorStop(1, "rgba(120,230,240,0)");
      ctx.fillStyle = glow;
      ctx.beginPath();
      ...
      ctx.textAlign = "right";
      ctx.fillStyle = "rgba(140,200,210,0.9)";
